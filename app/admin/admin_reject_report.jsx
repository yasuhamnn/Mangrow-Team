...
import {
  getReportForRejection,
  rejectReportWithReason,
  REPORT_REJECTION_PRESETS,
} from '../utils/admin/adminReportRejectionBackend'

export default function AdminRejectReport() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true)
        const data = await getReportForRejection(reportId)
  ...
  const onSubmit = async () => {
    ...
    if (category === 'other' && !customNote.trim()) {
      Alert.alert('Required', 'Please explain why this report was rejected.')
      return
    }

    setSubmitting(true)
    try {
      await rejectReportWithReason({
        reportId,
        category,
        customNote,
      })
      Alert.alert('Rejected', 'The volunteer will be notified with your feedback.', [
        { text: 'OK', onPress: () => router.replace('/admin/admin_verify') },
      ])
    } catch (e) {
      Alert.alert('Error', e?.message || 'Failed to reject report.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.summaryCard}>
          ...
          />
          <View style={{ flex: 1 }}>
            <Text style={styles.species}>{report?.species || 'Unknown Species'}</Text>
            <Text style={styles.location}>{report?.formatted_address || 'Unknown location'}</Text>
            <Text style={styles.volunteer}>Volunteer: {report?.volunteer_name || '—'}</Text>
          </View>
        </View>

        <Text style={styles.sectionTitle}>Why is this report being rejected?</Text>
        <Text style={styles.sectionHint}>
          This message is private and will be shown to the volunteer in their app.
        </Text>

        <View style={styles.presetList}>
          {REPORT_REJECTION_PRESETS.map((preset) => {
        ...
        <TextInput
          ...
          multiline
          placeholder="Tell the volunteer what to improve for their next submission..."
