...
export async function getUnreadAdminNotificationsCount() {
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return 0
...
export function subscribeToAdminNotifications(userId, onChange, scope = 'default') {
  const channelName = `admin-notifications-${userId}-${scope}`
...
export async function markNotificationReadById(notificationId) {
  ...
  await markAdminNotificationAsRead(notificationId).catch(console.error)
}
