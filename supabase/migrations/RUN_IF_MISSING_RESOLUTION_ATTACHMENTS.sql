...
--   relation "public.resolution_attachments" does not exist
--
-- Your SQL EDITOR QUERY.txt has `resolutions` but the app uses:
--   - report_resolutions  (main resolution submissions)
--   - resolution_attachments  (extra proof photos)
-- =============================================================================

-- 1) App resolution table (migrates rows from legacy `resolutions` if you have it)
...
BEGIN
  IF EXISTS (
    ...
    SELECT
      r.id, r.report_id, r.user_id, r.description, r.image_url,
      r.latitude, r.longitude, r.status, COALESCE(r.created_at, now())
    FROM public.resolutions r
    WHERE NOT EXISTS (SELECT 1 FROM public.report_resolutions rr WHERE rr.id = r.id);
  END IF;
END $$;

-- 2) Extra proof photos per resolution
...
CREATE POLICY "Volunteers insert resolutions"
  ON public.report_resolutions FOR INSERT TO authenticated
...
CREATE POLICY "Read resolutions for visible reports"
  ON public.report_resolutions FOR SELECT TO authenticated
  USING (
    auth.uid() = user_id
...
CREATE POLICY "Admins update resolutions"
  ON public.report_resolutions FOR UPDATE TO authenticated
...
CREATE POLICY "Volunteers insert resolution attachments"
  ON public.resolution_attachments FOR INSERT TO authenticated
  WITH CHECK (
    EXISTS (
...
CREATE POLICY "Read resolution attachments"
  ON public.resolution_attachments FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.report_resolutions rr
...
-- 4) Storage bucket for resolution photos
